...
var aheadUsed = map[string]string{
	...
	"LID-4":  "game.js drawRelief(): /tiles/hillshade/{z}/{x}/{y}.png fetched directly (CORS, 1 y cache), re-centred + 'overlay' composite under the sprites; replaces the per-parcel slope/aspect tint when tiles cover the view",
	...
	"GW-1":   "server: GET /api/water/point, wellQuote() → POST /api/dig-well (water.go); game.js wellButtonHTML()/doDigWell() + drawWells() sprite + popup Brunnen row",
	"GW-2":   "server: GET /api/water/points + /api/water/station/{id}; game.js loadGwPoints() per tile, drawGwStations() pixel sprites (zoom ≥ 15), openStation() popup with 30-year pixel chart",
	"GW-3":   "server: droughtFrom() in dossier.go scales harvest yield; game.js field-economy popup row + harvestToast() ☀️ malus",
	"GW-4":   "server: now block → droughtState in /api/dossier; game.js updateWaterChip() HUD chip #water-chip + sidebar row + Chronik Wasser tab + Herald 'drought' hint",
